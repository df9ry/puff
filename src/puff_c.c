...
void InitGraph (int32_t *GraphDriver, int32_t *GraphModus, char *PathToDriver)
{
}
...
void Bar (int32_t X1, int32_t Y1, int32_t X2, int32_t Y2) 
{
   int h;
...
void Rectangle (int32_t X1, int32_t Y1, int32_t X2, int32_t Y2) 
{
   int h;
...
void Arc (int32_t X, int32_t Y, int32_t start, int32_t stop, int32_t radius) 
{
  XDrawArc (dpy, pm, gc, Ox + X - radius, Oy + Y - radius, radius * 2,
            radius * 2,
            start*64, stop*64);
   drawcounter++;
}

void FillEllipse (int32_t X, int32_t Y, int32_t Xradius, int32_t Yradius) 
{
  XFillArc (dpy, pm, gcfill, Ox + X - Xradius, Oy + Y - Yradius, Xradius * 2,
            Yradius * 2,
            0, 64*360);
  XDrawArc (dpy, pm, gc, Ox + X - Xradius, Oy + Y - Yradius, Xradius * 2,
            Yradius * 2,
            0, 64*360);
   drawcounter++;
}
...
void FloodFill (int32_t X, int32_t Y, int32_t BorderColor) 
{
   ...
   */
   ...
   if (X == 0 || Y == 0)
     return; // FloodFill is called 4 times by the original
             // pascal code, this check only leaves the last one
...
void SetTextJustify (int32_t Horizontal, int32_t Vertical) 
{
...
void SetViewPort (int32_t X1, int32_t Y1, int32_t X2, int32_t Y2, int8_t Clip) 
{
   Ox = X1;
   Oy = Y1;
   vp.x1=X1;
...
void crtWindow (int32_t X1, int32_t Y1, int32_t X2, int32_t Y2)
{
   crtXmin=X1;
   crtYmin=Y1;
   crtXmax=X2;
...
void process_event(XEvent *xe)
{
   ...
   switch (xe->type) {
      case KeyPress:
         ...
         // first, consider the raw keysyms, to catch a few combinations of
         // keys with shift / alt switch (keysym)
         {
         case XK_1:
           if (xe->xkey.state & ShiftMask)
            ...
            // for the rest, first process the keysym further, in which things
            // like shift, ctrl and numlock are taken into account
         ...
         switch (keysym) {
            case XK_KP_Up:
            case XK_Up:
              key = (xe->xkey.state & ShiftMask) ? 184 : 200;
              return;
            case XK_KP_Down:
            case XK_Down:
              key = (xe->xkey.state & ShiftMask) ? 178 : 208;
              return;
            case XK_KP_Left:
            case XK_Left:
              key = (xe->xkey.state & ShiftMask) ? 180 : 203;
              return;
            case XK_KP_Right:
            case XK_Right:
              key = (xe->xkey.state & ShiftMask) ? 182 : 205;
              return;
            case XK_KP_Page_Up:
            case XK_Prior:
              key = 201; /* page up ? */
            ...
            case XK_F4:
              ...
              return;
            case XK_F5:
              key = (xe->xkey.state & ShiftMask) ? 216 : 191;
              return;
            case XK_F6:
              key = 192;
              return;
      ...
      case ConfigureNotify: 
         {
            ...
            // we notify the pascal code about the resize by sending a
            // "virtual" keypress
            key = 255;
            // this constant is screenresize in the pascal code
            // need to update the background pixmap
            Pixmap oldpm=pm;
            pm = XCreatePixmap (dpy, RootWindow (dpy, scr), ScreenWidth,
                                ScreenHeight, wa.depth);
...
void ClrScr(void)
{
   ...
   if (y==476) y=480;
   XFillRectangle (dpy, pm, gcclear, 8 * (crtXmin - 1), 14 * (crtYmin - 1),
                   8*(crtXmax-crtXmin+1), y);
   XClearWindow(dpy,w);
   XFlush(dpy);
}
...
void write_other(int x, int y, int c)
{
   switch (c) {
      ...
      case 27:     /* left arrow */
         ...
         break;
      case 132: /* ground; although the definition has been commented out in
                   pfun1_21.pas, it is still used in pfmsc_21.pas (and
                   displayed incorrectly under DOS, it seems) */
        y += 6;
        x += 3;
        XDrawLine (dpy, pm, gctext, x - 3, y, x + 3, y);
        y += 2;
        XDrawLine (dpy, pm, gctext, x - 2, y, x + 2, y);
        y += 2;
        XDrawLine (dpy, pm, gctext, x - 1, y, x + 1, y);
        break;
      ...
      case 235:    /* delta */
         ...
         y++;
         XDrawPoint (dpy, pm, gctext, x + 1, y);
         XDrawPoint (dpy, pm, gctext,
                     x-1,y);
         y++;
         XDrawPoint (dpy, pm, gctext, x + 2, y);
         XDrawPoint (dpy, pm, gctext,
                     x-2,y);
         y++;
         XDrawPoint (dpy, pm, gctext, x + 3, y);
         XDrawPoint (dpy, pm, gctext,
                     x-3,y);
...
void getpascalcolours()
{
   ...
   for (i=0;i<16;i++) {
      if (i>=8) on=65535;
        ...
        on = 49052; // exception to make the much-used grey colour
                    // a bit brighter
      else on=32768;
      xc.blue = (i&1)?on:0;
      xc.green = (i&2)?on:0;
...
void init_x(void)
{
   ...
   if (dpy==NULL) {
       fprintf (stderr,
           "Could not open display; is the DISPLAY variable set correctly?\n");
      exit(1);
   }
   ...
   // create our window
   w = XCreateWindow (dpy, RootWindow (dpy, scr), -1, -1, width, height, 0,
                      CopyFromParent, InputOutput, CopyFromParent, 0, NULL);
   XMapWindow(dpy,w);
   XStoreName(dpy,w,"puff");

   // create a background pixmap, that we're actually going to use for
   // everything, because then the xserver takes care of redrawing the window
   // when it is uncovered
   XGetWindowAttributes(dpy,w,&wa);
   pm=XCreatePixmap(dpy, RootWindow(dpy,scr),wa.width,wa.height,wa.depth);
   XSetWindowBackgroundPixmap(dpy,w,pm);

   // tell the window manager that we don't want to be resized to less than
   // 640x480 (the old VGA resolution)
   ...
   for (i = 0; i < 16; i++)
     boxes[i] = XCreatePixmap (dpy, RootWindow (dpy, scr), 32, 32,
                               wa.depth);
   ...
   XSetLineAttributes (dpy, gcfill, 1000, LineSolid, CapRound, JoinRound);
   // very thick line, see FloodFill() for explanation

   // clear the screen
   XFillRectangle(dpy, pm, gcfill, 0,0,ScreenWidth,ScreenHeight);
   XClearWindow(dpy,w);

   // load an 8x13 font; the VGA font on which the partitioning of the screen
   // is based is an 8x14 font
   fontinfo=XLoadQueryFont(dpy,"8x13");
   XSetFont(dpy,gctext,fontinfo->fid);
   XSetFont(dpy,gc,fontinfo->fid);
   fontyoffs=fontinfo->ascent;

   // allocate a set of colours corresponding to those of the VGA mode
   // originally used by puff
   getpascalcolours();
...
void GetBox(int32_t bn, int32_t x,int32_t y, int32_t width, int32_t height)
{
   XCopyArea(dpy, pm, boxes[bn], gc, x, y, width, height, 0, 0);
}

void PutBox(int32_t bn, int32_t x, int32_t y, int32_t width, int32_t height)
{
